...
pub mod on_chain_calculator {
    ...
    // Implement the `update_authority` function in the same manner as the update functions above.
    ...
    pub fn subtraction(ctx: Context<Compute>) -> Result<()> {
        ...
        let result: Option<i32> = if operand_x >= operand_y {
            Some(operand_x - operand_y)
        } else {
            None
        };

        // Emit the operands with the result into logs
        emit!(CalculatorEvent {
            x: operand_x,
            y: operand_y,
            result,
    ...
    pub fn division(ctx: Context<Compute>) -> Result<()> {
        ...
        // Ensure operand_y is not zero to avoid division by zero
        //if operand_y == 0 {
        //     return Err();
        //}
    ...
    // Implement other Calculator functions in the same manner as the addition function above.
